...
def core_model(case_dic, tech_list):
    ...
    for tech_dic in tech_list:
        ...
        # Shfting demand
            ...
            constraints += [ dispatch >= 0. ]
            node_balance[node_from] += -dispatch
            energy_shifted = cvx.Variable(num_time_periods) 

            sum_transferred = cvx.Variable(num_time_periods)
            for i in range(num_time_periods):
                new_var_tmp = cvx.Variable(num_time_periods)
                one_day_range_idx = np.arange(i-2, i+2, 1)%num_time_periods
                constraints += [new_var_tmp >= 0]
                constraints += [new_var_tmp <= capacity]
                constraints += [cvx.sum(new_var_tmp) == dispatch[i]]
                constraints += [cvx.sum(new_var_tmp[one_day_range_idx]) == dispatch[i]]
                constraints += [sum_transferred == sum_transferred + new_var_tmp]
            node_balance[node_from] += sum_transferred

            # sum_transferred = cvx.Variable(num_time_periods)
            # for i in range(num_time_periods):
            #     one_day_range_idx = np.array(np.arange(i-2, i+2, 1)%8760)
            #     new_var_tmp = cvx.Variable(len(one_day_range_idx))
            #     constraints += [new_var_tmp >= 0]
            #     constraints += [new_var_tmp <= capacity]
            #     constraints += [cvx.sum(new_var_tmp) == dispatch[i]]
            #     constraints += [sum_transferred[one_day_range_idx.astype('int')] == sum_transferred[one_day_range_idx.astype('int')] + new_var_tmp ]
            # node_balance[node_from] += sum_transferred
            ...
            for i in range(num_time_periods): 
                constraints += [energy_shifted[(i+1) % num_time_periods] == energy_shifted[i] + sum_transferred[i] - dispatch[i]]
                constraint_list += [tech_name + ' storage_balance_step_'+str(i).zfill(5)]

            capacity_dic[tech_name] = capacity
            dispatch_dic[tech_name] = dispatch
            dispatch_dic[tech_name] = sum_transferred
            stored_dic[tech_name] = energy_shifted
